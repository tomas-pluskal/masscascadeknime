...
import uk.ac.ebi.masscascade.alignment.FeatureBinTable;
 ...
 */
public class FeatureViewerNodeView extends NodeView<FeatureViewerNodeModel> {

	private final static Dimension PREF_DISPLAY_SIZE = new Dimension(900, 600);

	private FeatureBinTable table;
	 ...
	 */
	protected FeatureViewerNodeView(final FeatureViewerNodeModel nodeModel) {
	...
	private void loadData(int[] selectedRows) {
		...
		for (int selectedRow : selectedRows) {
			...
			Map<String, Feature> profiles = getNodeModel().getModel().getProfilesForRow(index);
			for (Map.Entry<String, Feature> entry : profiles.entrySet()) {
				if (selectedRows.length == 1) color = graphColor.nextColor();
